...
public class HelloTest {
	...
	private String addStore = String.format("geowave config addstore %s --gwNamespace %s -t hbase --zookeeper sandbox.hortonworks.com:2181", vStore, vSpace);
	private String addStore_KDE = String.format("geowave config addstore %s --gwNamespace %s -t hbase --zookeeper sandbox.hortonworks.com:2181", vStoreKDE, vSpaceKDE);
	...
	private String runKDE = String.format("hadoop jar /usr/local/geowave-0.9.5-hdp2/tools/geowave-tools-0.9.5-hdp2.jar analytic kde --featureType gdeltevent --minLevel 5 --maxLevel 26 --minSplits 32 --maxSplits 32 --coverageName %s --hdfsHostPort sandbox.hortonworks.com:8020 --jobSubmissionHostPort sandbox.hortonworks.com:8032 --tileSize 1 %s %s", vCoverageKDE, vStore, vStoreKDE);
	
	private String addStore_raster = String.format("geowave config addstore -t hbase -z sandbox.hortonworks.com:2181 %s --gwNamespace %s", rStore, rSpace);
